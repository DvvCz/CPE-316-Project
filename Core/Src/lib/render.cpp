...
void Render::LCDRenderer::drawRect(uint16_t x, uint16_t y, uint16_t width, uint16_t height) {
  if (currentTexture.has_value()) {
    ...
    case LCDTextureWrap::Fill:
      lcd->drawRectTextured(x, y, width, height, currentTexture.data, currentTexture.width, currentTexture.height,
...
*/
void drawSingleCircleOutline(Display::Controller* lcd, uint16_t x, uint16_t y, uint16_t radius,
                             Render::LCDColor currentColor) {
  int x0 = 0;
  int y0 = radius;
  int d = 3 - 2 * radius;
...
void Render::LCDRenderer::drawLine(uint16_t x0, uint16_t y0, uint16_t x1,
                                   uint16_t y1) { // this function only works for horizontal and vertical lines
  if (y0 == y1) {                                 // Horizontal
...
void Render::LCDRenderer::drawRectOutline(uint16_t x, uint16_t y, uint16_t width, uint16_t height, uint8_t thickness) {
  if (thickness == 1) {
    ...
    for (uint8_t i = 0; i < thickness; i++) {
      drawRectOutline(x + i, y + i, width - 2 * i, height - 2 * i, 1);
    }
  }
}
